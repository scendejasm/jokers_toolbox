function New-EmployeeOnboardUser {
	...
	process {
		...
		$DefaultGroup = 'TEST Users'
			
		$Username = "$($FirstName.SubString(0, 1))$LastName"
		$EaActionBefore = $ErrorActionPreference
		...
		$ErrorActionPreference = $EaActionBefore
		$NewUserParams = @{
		...
		Add-ADGroupMember $DefaultGroup $Username 
        $Username
	}
}
...
function New-EmployeeOnboardComputer {
	...
	param (
		[string]$Computername,
		[string]$Location = 'Company Computers'
	)
	process {
